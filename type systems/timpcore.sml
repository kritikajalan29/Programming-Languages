...
(* function [[ty]], checks type of expression given $\itenvs$ ((prototype)) 348 *)
      | ty (AAT (a, i))        = 
             (case (ty a, ty i)
                of (ARRAYTY(tau_elem), INTTY) => tau_elem
                | (ARRAYTY(_), _) => 
                                  raise TypeError ("Index of an array has type "
                                   ^ typeString (ty i) ^ ", which should be " ^ 
                                   typeString INTTY)
                | (_, INTTY) => 
                                raise TypeError ("ArrayAt expects an Array but" 
                                ^ "gets" ^ typeString (ty a))
                | (_, _) => raise TypeError "Error"
              ) 
      | ty (APUT (a, i, e))    = 
             (case (ty a, ty i, ty e)
                of (ARRAYTY(tau_array), INTTY, tau_elem) => 
                                if eqType (tau_elem, tau_array) then 
                                  tau_elem 
                                else 
                                  raise TypeError ("ArrayPut expects an element"
                                ^ "of type" ^ typeString tau_array ^ "but gets"
                                ^ typeString (ty e))
                | (_, INTTY, _) => 
                                raise TypeError ("ArrayPut expects an Array but"
                                ^ "gets" ^ typeString (ty a))
                | (ARRAYTY(tau_array), _, tau_elem) => 
                                raise TypeError ("Index of an array has type "
                                  ^ typeString (ty i) ^ ", which should be " ^
                                  typeString INTTY)
                | (_, _, _) => raise TypeError "Error"
                
              )
      | ty (AMAKE (len, init)) = 
              (case (ty len, ty init)
                of (INTTY, tau_elem) => ARRAYTY tau_elem
                | (_, _) => raise TypeError ("ArrayMake expects a array size" ^
                                "of type" ^  typeString INTTY  ^ "but gets" ^
                                typeString (ty len))
              )
      | ty (ASIZE a)           = 
             (case ty a
                of (ARRAYTY(tau_elem)) => INTTY
                | (_) => 
                      raise TypeError ("ArraySize expects an Array but gets" 
                              ^ typeString (ty a))
             )
(* type declarations for consistency checking *)
