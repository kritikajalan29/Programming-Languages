...
(* typeof (e, delta, gamma) takes in an expression e, the kind environment delta
and the typing environment gamma. typeof typechecks e under the given kind and 
typing environment and returns the type of the expression. *)


(* type checking for {\tuscheme} ((prototype)) 366 *)
fun typeof (e, delta, gamma) = 
  let fun ty (LITERAL l) = 
              let fun literal (NUM _) = inttype
                    | literal (BOOLV _) = booltype
                    | literal (SYM _) = symtype
                    | literal (NIL)  = FORALL(["'a"], listtype tvA)
                    | literal (PAIR (v, NIL)) = listtype (ty (LITERAL v))
                    | literal (PAIR (v, v_prime)) = 
                        if eqType (listtype (ty (LITERAL v)),  
                                       ty (LITERAL v_prime))
                        then  listtype (ty (LITERAL v))
                        else raise TypeError "Ill-Typed"
                    | literal (PRIMITIVE _) = raise TypeError "Ill-Typed"
                    | literal (CLOSURE _) = raise TypeError "Ill-Typed"
                    | literal (ARRAY _) = raise TypeError "Ill-Typed"
              in literal l 
              end
        | ty (VAR x)        =  find (x, gamma)
        | ty (SET (x, e))   = if eqType (find (x, gamma), ty e) 
                              then ty e 
        ...
        | ty (IFX (e1, e2, e3))  = 
              let val tau_e1 = ty e1
                  val tau_e2 = ty e2
                  val tau_e3 = ty e3                   
              in  if (eqType(booltype, tau_e1)) andalso (eqType(tau_e2, tau_e3))
                  then tau_e2 
                  else raise TypeError "Ill-Typed, IFX"
              end 
        | ty (WHILEX (e1, e2)) = 
              if eqType (ty e1, booltype) andalso eqType (ty e2, ty e2)
              then unittype
              else raise TypeError "Ill-Typed, WHILEX"
        | ty (BEGIN [])   = unittype
        | ty (BEGIN es)   = if eqTypes (map ty es, map ty es)
                            then ty (List.last es)
                            else 
                            raise TypeError "Ill-Typed, BEGIN"
        | ty (APPLY (exp, es))   = 
              (case (ty exp, map ty es)
                  of (FUNTY (tau_args, tau_ret), tau_es) =>
                      if eqTypes(tau_args, tau_es)  
                      then tau_ret
                      else 
                      raise TypeError "Ill-Typed, APPLY ty args != ty es"
                  | (_, _) => 
                      raise TypeError "Ill-Typed, APPLY not FUNTY"
              )
        | ty (LETX (LET, binds, e))  =  
              typeof (e, delta, gamma <+> mkEnv (map fst binds, 
                                          map ty (map snd binds)))
        | ty (LETX (LETSTAR, [], e)) = ty e
        | ty (LETX (LETSTAR, b::bs, e)) = 
              ty (LETX (LET, [b], LETX (LETSTAR, bs, e)))
        | ty (LETRECX (binds, e)) = 
            let val e_is = map snd binds
                val tau_is = map (fn (arg) => asType (arg, delta)) 
                                                  (map snd (map fst binds))
                val name_is = map fst (map fst binds)
                val extended_env = gamma <+> mkEnv(name_is, tau_is)
            in if eqTypes (map (fn (e_i) => typeof (e_i, delta, extended_env))
                                                                 e_is, tau_is)
                then typeof (e, delta, extended_env)
                else raise TypeError "Ill-Typed, LETREC"
            end 
        | ty (LAMBDA ((args, e))) = 
            let val arg_taus = map snd args
                val extended_env = gamma <+> mkEnv(map fst args, arg_taus)
            in FUNTY (map (fn (arg) => asType (arg, delta)) arg_taus, 
                                            typeof (e, delta, extended_env))
            end
        | ty (TYLAMBDA (xs, e)) = 
            let fun not_free_in (alpha) = 
                  List.all (fn (free_var) => not (alpha = free_var)) 
                                                  (freetyvarsGamma gamma)
            in if List.all (fn (alpha) => not_free_in alpha) xs then 
                let val new_delta = foldr (fn (x, acc) => bind (x, TYPE, acc))
                                            delta xs
                    val tau_e = typeof (e, new_delta, gamma)
                in FORALL(xs, tau_e)  
                end
                else 
                raise TypeError "Ill-Typed, tylambda"
            end
        | ty (TYAPPLY  (e, tyex_list)) = 
          (case ty e
            of (FORALL(xs, tau)) => let val tau_is = 
                                      map (fn (tau_i) => asType(tau_i, delta)) 
                                           tyex_list
                                    in instantiate(FORALL(xs, tau), tyex_list,
                                                                    delta) 
                                    end
            | (_) => raise TypeError "Ill-Typed"
          )
  in ty e
  end 


(* typdef (def, delta, gamma) takes in a definition def, the kind environment
delta, and the typing environment gamma. typdef typchecks def and returns a new
typing environment containing the name that is defined and a string  
representing the type of the definition *)

fun typdef (def, delta, gamma) = 
  let fun tyd (VAL (x, e)) = 
            let val tau_e = typeof (e, delta, gamma) 
            in (bind (x, tau_e, gamma), typeString tau_e)
            end 
        | tyd (VALREC (x, tyex, e)) = 
            let val tau = asType(tyex, delta)
                val extended_env = bind (x, tau, gamma)
                val tau_e = typeof (e, delta, extended_env)
            in  if eqType (tau, tau_e) 
                then (extended_env, typeString tau)
                else raise TypeError "Ill-Typed, VALREC"
            end
        | tyd (EXP (e)) = tyd (VAL ("it", e))
        | tyd (DEFINE (x, tyex, lambda_e)) = 
            let val name_taus = fst lambda_e
                val arg_taus = map snd name_taus
            in tyd (VALREC (x, FUNTY(arg_taus, tyex), LAMBDA lambda_e))
            end
  in tyd def
  end 
