...
fun solve c = (case c
                 of TRIVIAL     => idsubst
                  ...
                  | t1 ~ t2  => 
                    (case (t1, t2)
                       of (TYVAR a1, TYVAR a2)   => a1 |--> t2
                        | (TYVAR a1, TYCON a2)   => a1 |--> t2
                        | (TYVAR a1, CONAPP _) => 
                            if member a1 (freetyvars t2)
                            then unsatisfiableEquality (t1, t2)
                            else a1 |--> t2
                        | (TYCON a1, TYVAR a2)   => a2 |--> t1
                        | (TYCON a1, TYCON a2)   => 
                            if eqTycon (t1, t2) then idsubst
                            else unsatisfiableEquality (t1, t2)
                        | (TYCON a1, CONAPP a2)  => 
                            unsatisfiableEquality (t1, t2)
                        | (CONAPP _, TYVAR a2)  => 
                            if member a2 (freetyvars t1)
                            then unsatisfiableEquality (t1, t2)
                            else a2 |--> t1
                        | (CONAPP a1, TYCON a2)  =>
                            unsatisfiableEquality (t1, t2)
                        | (CONAPP (a1, a1s), CONAPP (a2, a2s)) =>
                            let val c1 = a1 ~ a2
                                val cs = ListPair.foldlEq (fn (b1, b2, acc) =>
                                                           acc /\ (b1 ~ b2))
                                                           c1 (a1s, a2s)
                            in solve cs
                            end
...
val hasGoodSolution = not o hasNoSolution : con -> bool
fun solutionEquivalentTo (c, theta) = eqsubst (solve c, theta)

(* UNIT TESTING *)

(* TYVAR to x *)
val () = Unit.checkAssert "a ~ b passes"
         (fn () => solutionEquivalentTo (alpha ~ beta, "a" |--> beta))
val () = Unit.checkAssert "a ~ int passes"
         (fn () => solutionEquivalentTo (alpha ~ inttype, "a" |--> inttype))
val () = Unit.checkAssert "a ~ bool passes"
         (fn () => hasGoodSolution (alpha ~ booltype))
val () = Unit.checkAssert "a ~ list b passes"
         (fn () => hasGoodSolution (alpha ~ listtype beta))
val () = Unit.checkAssert "a ~ list a passes"
         (fn () => hasNoSolution (alpha ~ listtype alpha))

(* TYCON to x *)
val () = Unit.checkAssert "bool ~ bool solved by idsubst"
         (fn () => solutionEquivalentTo (booltype ~ booltype, idsubst))
val () = Unit.checkAssert "bool ~ int not solved"
         (fn () => hasNoSolution (booltype ~ inttype))
val () = Unit.checkAssert "int ~ int list not solved"
         (fn () => hasNoSolution (inttype ~ listtype inttype))

(* CONAPP to x *)
val () = Unit.checkAssert "list b ~ a passes"
         (fn () => hasGoodSolution (listtype beta ~ alpha))
val () = Unit.checkAssert "list a ~ a passes"
         (fn () => hasNoSolution (listtype alpha ~ alpha))
val () = Unit.checkAssert "int list ~ int not solved"
         (fn () => hasNoSolution (listtype inttype ~ inttype))
val () = Unit.checkAssert "int list ~ int list solved"
         (fn () => hasGoodSolution (listtype inttype ~ listtype inttype))
val () = Unit.checkAssert "int list ~ bool list not solved"
         (fn () => hasNoSolution (listtype inttype ~ listtype booltype))
val () = Unit.checkAssert "int list ~ int list solved"
         (fn () => hasGoodSolution (funtype ([inttype, booltype], inttype) ~
                                    funtype ([inttype, booltype], inttype)))
val () = Unit.checkAssert "int list ~ int list solved"
         (fn () => hasNoSolution (funtype ([booltype, booltype], inttype) ~
                                  funtype ([inttype, booltype], inttype)))
val () = Unit.checkAssert "int list ~ int list solved"
         (fn () => hasNoSolution (funtype ([inttype, inttype], inttype) ~
                                  funtype ([inttype, booltype], inttype)))
val () = Unit.checkAssert "int list ~ int bool pair no solution"
         (fn () => hasNoSolution (funtype ([inttype, inttype], inttype) ~
                                  pairtype (inttype, booltype)))

(* TRIVIAL *)
val () = Unit.checkAssert "int list ~ int list solved"
         (fn () => hasGoodSolution TRIVIAL)
      ...
      fun literal v  =
        (case v
           of NUM _           => (inttype, TRIVIAL)
            ...
            | NIL             => (listtype (freshtyvar ()), TRIVIAL)
            | PAIR (x, xs)    => let val (ty1, c1) = literal x
                                     val (ty2, c2) = literal xs
                                     val c = c1 /\ c2 /\ (listtype ty1) ~ ty2
                                  in (listtype ty1, c)
                                  end 
            | CLOSURE _       => raise BugInTypeInference "" 
            | PRIMITIVE _     => raise BugInTypeInference "")
...
(* function [[ty]], to infer the type of a \nml\ expression, given [[Gamma]] 438c *)
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3)) =
            let val (ty1, c1) = ty e1
                val (ty2, c2) = ty e2
                val (ty3, c3) = ty e3
                val new_c = (ty1 ~ booltype)
                            /\ (ty2 ~ ty3)
                            /\ c1 /\ c2 /\ c3
            in (ty3, new_c)
            end
        ...
        | ty (BEGIN es) = 
            let val (tys, c) = typesof (es, Gamma)
                val last_ty = List.last tys
            in (last_ty, c)
            end
        | ty (LAMBDA (formals, body)) =
            let val freshtyvars = List.map (fn (_) => freshtyvar ()) formals
                val new_gamma = ListPair.foldrEq
                                (fn (x, a1, env) => bindtyscheme
                                                    (x, FORALL ([], a1), env))
                                                    Gamma (formals, freshtyvars)
                val e = typeof (body, new_gamma)
            in (funtype (freshtyvars, fst e), snd e)
            end 

            (* typesof for es in bs*)

        | ty (LETX (LET, bs, body))   = 
            let val (tys, c)  = typesof(map snd bs, Gamma)
                val theta  = solve c
                val alphas = map TYVAR (inter(dom theta, freetyvarsGamma Gamma))
                val c_prm  = conjoinConstraints (map
                                                 (fn (a) => a ~ tysubst theta a)
                                                 alphas)
                val sigmas = map (fn (tau) => generalize (tysubst theta tau,
                                              union (freetyvarsGamma Gamma,
                                              freetyvarsConstraint c_prm))) tys
                val g_ext  = ListPair.foldrEq (fn (x, s, acc) => bindtyscheme
                                                           (x, s, acc))
                                               Gamma
                                               (map fst bs, sigmas)
                val tau_cb = typeof (body, g_ext)
            in (fst tau_cb, c_prm /\ (snd tau_cb))
            end

        | ty (LETX (LETREC, bs, body)) = 
            let val freshtvs = List.map freshtyvar (map fst bs)
                val gamma_e  = ListPair.foldrEq
                               (fn (x, a1, env) => bindtyscheme
                                                   (x, FORALL ([], a1), env))
                                                   Gamma
                                                   (map fst bs, freshtvs)
                val (tys, c_e)    = typesof (map snd bs, gamma_e)
                val c         = c_e /\
                                conjoinConstraints (ListPair.map 
                                                    (fn (tau, a) => tau ~ a)
                                                    (tys, freshtvs))
                val theta = solve c
                val alphas = map TYVAR (inter (dom theta,
                ...
                val sigmas = map (fn (tau) => generalize (tysubst theta tau,
                                              union (freetyvarsGamma Gamma,
                                              freetyvarsConstraint c_prm))) 
                                              tys
                val g_ext  = ListPair.foldrEq (fn (x, s, acc) => bindtyscheme
                                                           (x, s, acc))
                                               Gamma
                                               (map fst bs, sigmas)
                val (tau, c_b) = typeof (body, g_ext)
            in (tau, c_prm /\ c_b) 
            end 
(* type declarations for consistency checking *)
