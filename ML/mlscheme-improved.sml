...
(******************************************************************************)
(*                                                                            *)
(*                       FREE VARIABLES FOR  \USCHEME                         *)
(*                                                                            *)
(******************************************************************************)

(* freeIn takes in a μScheme expression, exp and a variable name, name and 
returns a boolean value indicating whether there is a free variable of that name
in the expression exp*)

fun freeIn exp name = 
  let fun free (LITERAL v) = false
        | free (VAR x) = name = x
        | free (SET (x, e)) = (x = name) orelse (free e)
        | free (IFX (e1, e2, e3)) = List.exists free [e1, e2, e3]
        ...
        | free (LAMBDA (xs, e)) =   not (member name xs) andalso (free e)
        | free (LETX (LET, bs, e)) = List.exists free (map snd bs) orelse 
                                      ((free e) andalso not (member name 
                                                              (map fst bs)))
        | free (LETX (LETREC, bs, e)) = (List.exists free (map snd bs)  orelse 
                                          (free e))  andalso (not (member name 
                                                                (map fst bs)))
        | free (LETX (LETSTAR, [], e)) = free e
        | free (LETX (LETSTAR, b::bs, e)) = free (LETX (LET, [b], 
                                                        LETX (LETSTAR, bs, e)))
    in free exp
  end
...
local
   ...
   val set_x2 = SET ("x", VAR "y")
   val if_t = IFX (LITERAL (BOOLV true), VAR "y", VAR "x")
   val while_f = WHILEX (LITERAL (BOOLV false), VAR "y")
   val begin_ = BEGIN [LITERAL (BOOLV false), VAR "y", 
                                  IFX (LITERAL (BOOLV true), VAR "y", VAR "x")]
   val apply_ = APPLY (LITERAL (NUM 2), [VAR "y", IFX (LITERAL (BOOLV true), 
                                                            VAR "a", VAR "x")])
   val lambda_ = LAMBDA (["y", "x"], apply_)
   val lambda_1 = LAMBDA (["a", "b"], apply_)
   val lambda_y = LAMBDA (["y"], APPLY (VAR "equal?", [VAR "x", VAR "y"]))
   val let_ = LETX(LET, [("x1", LITERAL (NUM 3)), ("x2", LITERAL (NUM 4))], 
                                                            SET ("x", VAR "y"))
   val let_rec = LETX(LETREC, [("x1", LITERAL (NUM 3)), ("x2", VAR "x1")],
                                                            SET ("x", VAR "y"))
   val let_star = LETX(LETSTAR, [("x1", VAR "x2"), ("x2", VAR "x3")], 
                                                          SET ("x", VAR "y"))

 in
 val () =
     Unit.checkAssert "freeIn (literal) false"
     (fn () =>  not (freeIn (LITERAL (NUM 3)) "y"))

 val () =
     Unit.checkAssert "freeIn (var) false" 
     (fn () => not (freeIn (VAR "x") "y"))
 ...
 val () =
  ...
  val () =
     Unit.checkAssert "freeIn (set literal) false"
     (fn () => not (freeIn set_x1 "y"))

  val () =
     Unit.checkAssert "freeIn (set var) true"
     (fn () =>  freeIn set_x2 "y")
  
  val () = 
    Unit.checkAssert "freeIn (if_t) true"
     (fn () =>  freeIn if_t "y")

  val () = 
    Unit.checkAssert "freeIn (if_t) false"
     (fn () =>  not (freeIn if_t "z"))
  
  val () = 
    Unit.checkAssert "freeIn (while_f) true"
     (fn () =>  freeIn while_f "y")
  
  val () = 
    Unit.checkAssert "freeIn (while_f) false"
     (fn () =>  not (freeIn if_t "z"))
  
  val () = 
    Unit.checkAssert "freeIn (begin_) false"
     (fn () =>  not (freeIn begin_ "z"))
  ...
  val () = 
   ...
   val () = 
    Unit.checkAssert "freeIn (apply_) true"
     (fn () =>  freeIn apply_ "y") 

    val () = 
    Unit.checkAssert "freeIn (begin_) false"
     (fn () =>  not (freeIn apply_ "z"))

    val () = 
    Unit.checkAssert "freeIn (lambda_) false"
     (fn () =>  not (freeIn lambda_ "y"))
    
    val () = 
    Unit.checkAssert "freeIn (lambda_1) true"
     (fn () =>  freeIn lambda_1 "y")

    val () = 
    Unit.checkAssert "freeIn (lambda_y) true"
     (fn () =>  freeIn lambda_y "equal?")
    
    val () = 
    Unit.checkAssert "freeIn (let_) true"
     (fn () =>  freeIn let_ "x")
    
    val () = 
    Unit.checkAssert "freeIn (let_rec) false"
     (fn () =>  not (freeIn let_ "x1"))

    val () = 
    Unit.checkAssert "freeIn (let_) false"
     (fn () =>  not (freeIn let_rec "x1"))

    val () = 
    Unit.checkAssert "freeIn (let_rec) true"
     (fn () =>  freeIn let_rec "x")

    val () = 
    Unit.checkAssert "freeIn (let_star) true"
     (fn () =>  freeIn let_star "x")
    
    val () = 
    Unit.checkAssert "freeIn (let_star) true"
     (fn () =>  freeIn let_star "x2")
    
    val () = 
    Unit.checkAssert "freeIn (let_star) false"
     (fn () =>  not (freeIn let_star "x1"))
 end
...
(* improve takes in a pair consisting of a LAMBDA’s components (a list of 
formals, xs and a body exp) and an environment, and returns a better pair 
containing the same LAMBDA’s components paired with an environment that contains
only the free variables of the LAMBDA *)


fun improve ((xs, exp), env) = ((xs, exp), List.filter (fn (envir) => 
                                                  freeIn exp (fst envir)) env )
